...
namespace Application.Validators
{
    public class BaseActivityValidator<T, TDTO> : AbstractValidator<T> where TDTO : BaseActivityDTO
    {
        ...
        {
            RuleFor(x => selector(x).Title).NotEmpty().WithMessage("Title is required").MaximumLength(100).WithMessage("TItle must not exceed more than 100 characters");
            RuleFor(x => selector(x).Description).NotEmpty().WithMessage("Description is required").MaximumLength(100).WithMessage("TItle must not exceed more than 100 characters");
